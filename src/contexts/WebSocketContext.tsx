...
const EventWebSocketContext = createContext<EventWebSocketContextProps | undefined>(undefined);

const EventWebSocketProvider: React.FC<{ children: ReactNode }> = ({children}) => {
    const [isConnected, setIsConnected] = useState(false);
    const connector = useMemo(() => new EventsWebSocket(config.app.wws, ''), [config.app.wws]);

    const [priceChangeEvent, setPriceChangeEvent] = useState<PriceChangeEvent | null>(null)
    const [orderBookEvent, setOrderBookEvent] = useState<OrderBookEvent | null>(null)
    ...
    useEffect(() => {
        ...
        const handleMarketUpdate = (data: OrderBookEvent | PriceChangeEvent) => {
    ...
    return (<EventWebSocketContext.Provider value={{
        isConnected,
        subscribe: (assetsIds: string[]) => connector.send('subscribe', {assetsIds}),
        orderBookEvent,
        priceChangeEvent
    }}>
        {children}
...
const useEventWebSocket = (): EventWebSocketContextProps => {
    ...
    if (!context) {
        throw new Error('useEventWebSocket must be used within a EventWebSocketProvider');
    }
    return context;
};
